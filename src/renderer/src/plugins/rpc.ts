...
// plugin worker boundary (host uses even ids, worker odd). Pure module, no
// runes, unit-testable.

import type { RpcMessage, RpcError } from '../../../shared/plugins'
...
function codeOf(error: Error): RpcError['code'] {
  ...
  if (code === 'permission-denied' || code === 'cancelled' || code === 'invalid') return code
  return 'internal'
}
